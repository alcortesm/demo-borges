...
# where do we want to store the borges binary
BORGES_CMD="/tmp/borges"
# this needs to be run in order to download update version of the makefiles.
BORGES_CLEAN_CI_CMD="rm Makefile.main; rm -rf .ci"
BORGES_BUILD_CMD="make packages"
# where to find the binary generated by borges' build command
BORGES_BIN_PATH="bin/borges"
...
BORGES_RUN_PRODUCER="${BORGES_CMD} producer \
    --source=file --file=${BORGES_INPUT} \
    --queue=${BORGES_QUEUE}"
BORGES_RUN_CONSUMER="${BORGES_CMD} consumer \
    --queue=${BORGES_QUEUE} \
...
# create a borges binary at ${BORGES_CMD}
compile_borges() {
    echo [borges] compiling and installing...
    pushd ${GOPATH}/src/github.com/src-d/borges >/dev/null
    eval "${BORGES_CLEAN_CI_CMD}"
    eval "${BORGES_BUILD_CMD}"
    cp ${BORGES_BIN_PATH} ${BORGES_CMD}
    popd >/dev/null
}
